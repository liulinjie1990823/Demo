...
/**
 * An extension of a linear layout that supports the divider API of Android 4.0+. You can populate this layout with data that comes from a {@link android.widget.ListAdapter}
 */
public class LinearListView extends IcsLinearLayout {
	...
	/**
	 * When the current adapter is empty, the LinearListView can display a special view call the empty view. The empty view is used to provide feedback to the user that no data is available in this
	 * LinearListView.
	 * 
	...
	/**
	 * Update the status of the list based on the empty parameter. If empty is true and we have an empty view, display it. In all the other cases, make sure that the layout is VISIBLE and that the
	...
	/**
	 * Internal OnClickListener that this view associate of each of its children so that they can respond to OnItemClick listener's events. Avoid setting an OnClickListener manually. If you need it
	 * you can wrap the child in a simple {@link android.widget.FrameLayout}.
	 */
